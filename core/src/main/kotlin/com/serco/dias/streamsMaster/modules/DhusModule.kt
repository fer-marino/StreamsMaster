...
import com.serco.dias.streamsMaster.utils.StreamsMaster
...
class DhusModule: StreamModule {
    ...
    @Autowired lateinit var meterUtils: StreamsMaster

    override fun finalize(product: Product): Product {
        log.info("finalizing ${product.name} ")
        return product
    }
            ...
            try {
                ...
                for (entry in response.body!!.d.results) {
                    if(!Files.exists(Paths.get("uncompressed", entry.name))) {
                        ...
                        lastBatch.add(Product(entry.name, Date(), center, null, null,
                                mapOf("uuid" to entry.id, "ingestionDate" to entry.ingestionDate)))
